...
int main(int argc, char* argv[])
{
    ...
    int cnt_i = 0x00;
    char message[][64] = {"Hello world!", "nice to meet you", "my name is yb", "have a nice day"};
    ...
    for(cnt_i = 0 ; cnt_i < sizeof(message)/sizeof(message[0]) ; cnt_i ++)
    {
        write(clint_sock, message[cnt_i], sizeof(message[cnt_i]));
        printf("sending message: %s\n", message[cnt_i]);
    }
    close(clint_sock); //sending EOF to client
    close(serv_sock);
    // fwirte 
